...
import type { Cell, Rows } from "./TableNode";
...
function TableCell({
	...
	const { uiContainer } = useContext(CellContext);
			...
			editor.registerCommand<KeyboardEvent>(
				...
				(_event, targetEditor) => {
					...
					if (!isEditing && selection === null) {
						const extend = !!(_event && (_event as KeyboardEvent).shiftKey);
						const cellID = extend
