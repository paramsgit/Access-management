...
function useColorPickerStoreContext(consumerName: string): ColorPickerStore {
...
function useColorPickerContext(consumerName: string): ColorPickerContextValue {
...
const ColorPickerRoot = React.forwardRef<HTMLDivElement, ColorPickerRootProps>(
	(props, forwardedRef) => {
		...
		const stateRef = useLazyRef<ColorPickerStoreState>(() => initialColor);
		...
		const dirValue = useDirection(dir);

		return (
			<DirectionContext.Provider value={dirValue}>
...
const ColorPickerRootImpl = React.forwardRef<
	HTMLDivElement,
	ColorPickerRootImplProps
>((props, forwardedRef) => {
	const {
		value: valueProp,
		dir: dirProp,
...
ColorPickerRootImpl.displayName = "ColorPickerRootImpl";
...
const ColorPickerArea = React.forwardRef<HTMLDivElement, ColorPickerAreaProps>(
	(props, forwardedRef) => {
		...
		const AreaPrimitive: React.ElementType = asChild ? Slot : "div";
...
function ColorPickerHueSlider(props: ColorPickerHueSliderProps) {
	...
	return (
		...
		>
			...
			<SliderPrimitive.Thumb className="border-primary/50 bg-white focus-visible:ring-ring block size-4 rounded-full border shadow transition-colors focus-visible:ring-1 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50" />
		</SliderPrimitive.Root>
	);
}
...
function ColorPickerSwatch(props: ColorPickerSwatchProps) {
	...
	const SwatchPrimitive: React.ElementType = asChild ? Slot : "div";
...
function ColorPickerInput(props: ColorPickerInputProps): JSX.Element | null {
...
const inputGroupItemVariants = cva(
	"h-8 [-moz-appearance:_textfield] focus-visible:z-9999 focus-visible:ring-1 [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none",
	{
		variants: {
			position: {
		...
		(channel: "r" | "g" | "b" | "a", max: number, isAlpha = false) =>
			(event: React.ChangeEvent<HTMLInputElement>) => {
				...
				if (!Number.isNaN(value) && value >= 0 && value <= max) {
					...
					onColorChange({ ...color, [channel]: newValue } as ColorValue);
				}
			},
		[color, onColorChange],
	);
