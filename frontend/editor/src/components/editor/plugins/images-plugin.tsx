...
import {
	...
	$isImageNode,
	ImageNode,
} from "../../editor/nodes/image-node";
import type { ImagePayload } from "../../editor/nodes/image-node";
import { CAN_USE_DOM } from "../../editor/shared/can-use-dom";
...
export function InsertImageUploadedDialogBody({
	...
	onClick: (payload: InsertImagePayload) => void;
	convertFilesToImageUrl: (files: FileList | null) => Promise<string[] | null>;
}) {
	const [fileList, setFileList] = useState<FileList | null>(null);
	const [altText, setAltText] = useState("");
	const [_, setPreviewSrc] = useState("");

	const isDisabled = !fileList || altText.trim() === "";
	...
	const handleSubmit = async () => {
		...
		const uploadedUrls = await convertFilesToImageUrl(fileList);
		if (!uploadedUrls) throw new Error("Image upload failed");

		uploadedUrls.forEach((url) => {
			onClick({
				altText,
				src: url, // use uploaded URL not preview
			});
		});
	};
